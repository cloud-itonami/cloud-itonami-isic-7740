(ns ipleaseops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (IPLeaseAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the IPLeaseGovernor (:govern) and
  ...
  human operator/IP-licensing coordinator. The approver resumes with
...
(defn build
  ...
  `ipleaseops.store/Store`).
  opts:
    :advisor      -- an `ipleaseops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; IPLeaseAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; IPLeaseGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :license-id (:license-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :license-id (:license-id request) :by (:by approval)}]}
            {:disposition :hold
